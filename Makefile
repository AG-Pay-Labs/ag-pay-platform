...
.PHONY: help api-install api-migrate api-run api-lint seed-demo web-install web-run web-lint web-typecheck web-build lint test
...
api-run: ## Run the API with hot reload on port 8000.
	@cd $(API_DIR) && .venv/bin/uvicorn ag_platform_api.main:app --reload
